  ...
  return (
    <article className="space-y-6 px-4 py-5">
      ...
      <Section title="1. はじめに">
        <p>
          本ポリシーは、アプリ「NEWFIND」（Android パッケージ名: app.newfind.social。以下「本サービス」）が、利用者の情報をどのように取得・保存・利用するかを説明します。記載内容は、本サービスに実装されている機能に基づきます。
        </p>
        <p>
      ...
      <Section title="2. 取得する情報">
        ...
        <ul className="list-disc space-y-1 pl-5">
          ...
          <li>コメント</li>
          <li>いいね、Want、投稿の保存、フォロー関係</li>
          <li>共有操作の記録（投稿IDと、ログイン中の場合は利用者ID）</li>
          <li>商品の保存、および保存に伴う通知設定（アラームのオン/オフ）</li>
        </ul>
        <p>
        ...
        <ul className="list-disc space-y-1 pl-5">
          ...
          <li>
            Android アプリ: ログイン状態を端末内のローカルストレージに保存します
          </li>
        </ul>
        <p>
          位置情報、連絡先、広告ID、解析SDK、プッシュ通知トークンは、本サービスのコード上では取得していません。Android の利用権限はインターネット通信です。
        </p>
      </Section>
      ...
      <Section title="3. 利用目的">
        ...
        <ul className="list-disc space-y-1 pl-5">
          ...
          <li>プロフィール、投稿、コメント、いいね、フォロー、保存などのSNS機能の提供</li>
          <li>商品情報の表示と、保存した商品の管理</li>
          <li>不正利用への対応、運営上必要な管理（管理者による停止・削除を含みます）</li>
        </ul>
        <p>
          商品保存時に通知設定を保存することがありますが、アプリからの自動通知（プッシュ通知やメール配信）は、現時点では開始していません。
        </p>
      ...
      <Section title="4. 外部サービス">
        ...
        <ul className="list-disc space-y-1 pl-5">
          <li>
            Supabase（認証、データベース、ファイル保存）。アカウント、プロフィール、投稿、画像・動画、いいね、フォロー、保存、コメント等を保存します。
          </li>
      ...
      <Section title="8. アカウント削除・データ削除">
        ...
        <p>
          アカウントを削除すると、認証情報、プロフィール、自分が作成した投稿、コメント、いいね、Want、保存、フォロー、商品の保存、通知設定が削除されます。自分が作成した投稿の画像・動画およびプロフィール画像も、可能な範囲でストレージから削除します。他の利用者の投稿に対する共有記録は、投稿者情報を残さない形で匿名化される場合があります。
        </p>
        <p>
      ...
      <Section title="11. お問い合わせ">
        <p>
          本ポリシーに関するお問い合わせは、Google Play ストアのデベロッパー情報に記載の連絡先へご連絡ください。アプリ内に専用のお問い合わせフォームは、現時点ではありません。
        </p>
      </Section>
