...
export function PostOwnerMenu({
  ...
  function editPost(event: React.MouseEvent<HTMLButtonElement>) {
    event.preventDefault();
    event.stopPropagation();

    window.location.assign(`/p/${postId}/edit`);
  }
  ...
  return (
    <>
      <button
        ...
        className="absolute right-1 top-1 z-20 inline-flex h-8 w-8 items-center justify-center rounded-full bg-black/55 text-white"
      >
      ...
      {open
        ? createPortal(
            <div
              className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/40 pb-[env(safe-area-inset-bottom,0px)]"
              onMouseDown={(event) => {
                event.preventDefault();
                event.stopPropagation();
              }}
            >
              ...
              >
                <button
                  type="button"
                  onClick={editPost}
                  className="relative z-[100001] block w-full px-4 py-3.5 text-center text-sm font-semibold text-black bg-white"
                >
                  ✏️ 編集する
                </button>
